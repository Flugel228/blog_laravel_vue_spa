...
export default {
    ...
    methods: {
        ...
        destroyElement () {
            ...
            axios.delete(`http://localhost:8876/api/admin/categories/${this.id}`)
                .then( res => {
                    if (res.status === 200) {
                        alert(res.data.message)
                        window.location.href = 'http://localhost:8876/admin/categories/'
                    }
                })
                .catch( error => {
        ...
        goEdit () {
            window.location.href = `/admin/categories/${this.id}/edit`
        }
    },

    mounted () {
        this.id = window.location.pathname.replace('/admin/categories/', '')
        this.getElement()
    }
}
</script>
