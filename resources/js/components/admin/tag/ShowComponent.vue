...
export default {
    ...
    methods: {
        ...
        getElement () {
            axios.post(`http://localhost:8876/api/admin/tags/${this.id}`).
            then( res => {
        ...
        destroyElement () {
            ...
            axios.delete(`http://localhost:8876/api/admin/tags/${this.id}`)
                .then( res => {
                    if (res.status === 200) {
                        alert(res.data.message)
                        window.location.href = 'http://localhost:8876/admin/tags/'
                    }
                })
                .catch( error => {
        ...
        goEdit () {
            window.location.href = `/admin/tags/${this.id}/edit`
        }
    },

    mounted () {
        this.id = window.location.pathname.replace('/admin/tags/', '')
        this.getElement()
    }
}
</script>
